    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.embeddings.hybrid:
                self.embeddings.hybrid_model.root.conv.weight.copy_(
                    np2th(res_weight["conv_root/kernel"], conv=True))
                gn_weight = np2th(res_weight["gn_root/scale"]).view(-1)
                gn_bias = np2th(res_weight["gn_root/bias"]).view(-1)
                self.embeddings.hybrid_model.root.gn.weight.copy_(gn_weight)
                self.embeddings.hybrid_model.root.gn.bias.copy_(gn_bias)

                for bname, block in self.embeddings.hybrid_model.body.named_children():
                    for uname, unit in block.named_children():
...
class DecoderBlock(nn.Module):
    """
    use a skip-layer to build a top-down pathway to integrate two feature maps
    the generated features are computed as
    ret = conv(conv(cat(upx2(high), low)))
    """
